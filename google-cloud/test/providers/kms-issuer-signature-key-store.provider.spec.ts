...
const issuerKeyId = (alg: string) => `${md5(issuer)}-${alg}`
...
describe('kmsIssuerSignatureKeyStore', () => {
  ...
  it('should save an ES256 issuer key by importing it into KMS', async () => {
    ...
    assert.equal(
      ...
      kms.cryptoKeyPath(projectId, locationId, keyRingId, issuerKeyId('ES256'))
    )
  ...
  it('should share in-flight replacement import job creation across concurrent saves', async () => {
    ...
    try {
      const save1 = provider.save(issuer, 'ES256', {
        format: 'pem',
        declaredAlg: 'ES256',
        privateKey: privateKeyPem,
      })
      const save2 = provider.save(issuer, 'ES256', {
        format: 'pem',
        declaredAlg: 'ES256',
        privateKey: privateKeyPem,
      })
      ...
      while (
        ...
        if (attempts >= maxAttempts) {
          assert.fail(
            `Timed out waiting for replacement import job creation for baseImportJobId=${baseImportJobId} after ${maxAttempts} polling attempts`
          )
        }
        attempts += 1
  ...
  it('should fetch the latest enabled public key when KMS data is valid', async () => {
    ...
    const cryptoKeyName = kms.cryptoKeyPath(projectId, locationId, keyRingId, issuerKeyId('ES256'))
  ...
  it('should rethrow non-NOT_FOUND errors from fetch version lookup', async () => {
    ...
    await assert.rejects(provider.fetch(issuer, 'ES256'), /permission denied/)
  })
  ...
  it('should sign with the numerically latest enabled ES256 key version', async () => {
    ...
    await provider.sign(issuer, 'ES256', { iss: issuer }, { alg: 'ES256', typ: 'JWT' })
