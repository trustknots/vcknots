...
describe('firestorePreAuthorizedCodeStore', () => {
  ...
  const sampleCredentialConfigId = CredentialConfigurationId('TestCredentialConfig')
  ...
  it('should save and validate a code', async () => {
    const provider = firestorePreAuthorizedCodeStore({ app: mockApp })
    await provider.save(PreAuthorizedCode('test-code-123'), [sampleCredentialConfigId])
    const valid = await provider.validate(PreAuthorizedCode('test-code-123'))
    assert.deepEqual(valid, [sampleCredentialConfigId])
  })
  ...
  it('should delete a code', async () => {
    const provider = firestorePreAuthorizedCodeStore({ app: mockApp })
    await provider.save(PreAuthorizedCode('delete-me'), [sampleCredentialConfigId])
    await provider.delete(PreAuthorizedCode('delete-me'))
    const valid = await provider.validate(PreAuthorizedCode('delete-me'))
    assert.equal(valid, null)
  })

  it('should return null and delete an expired code', async () => {
    mock.timers.enable({ apis: ['Date'] })

    const provider = firestorePreAuthorizedCodeStore({ app: mockApp, expiresIn: 1000 })
    await provider.save(PreAuthorizedCode('expiring-code'), [sampleCredentialConfigId])
  ...
  it('should use default expiration of 5 minutes', async () => {
    ...
    const provider = firestorePreAuthorizedCodeStore({ app: mockApp })
    await provider.save(PreAuthorizedCode('default-expiry'), [sampleCredentialConfigId])
    ...
    assert.deepEqual(validBefore, [sampleCredentialConfigId])
  ...
  it('should use the correct Firestore document path', async () => {
    const provider = firestorePreAuthorizedCodeStore({ app: mockApp })
    await provider.save(PreAuthorizedCode('my-code'), [sampleCredentialConfigId])
