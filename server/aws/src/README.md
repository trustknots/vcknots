...
│   ├── authz.ts                # Lambda handler — Authorization Server
│   └── verifier.ts             # Lambda handler — Verifier
├── local/
│   ├── serve.ts                # HTTP server helper (uses @hono/node-server)
│   ├── issuer.ts               # Local entrypoint — Issuer (port 8081)
│   ├── authz.ts                # Local entrypoint — Authorization Server (port 8082)
│   └── verifier.ts             # Local entrypoint — Verifier (port 8083)
├── context/
