...
- **Registration**: the certificate is written on first startup, from `PRIVATE_KEY`/`CERTIFICATE` (or the `*_PATH` variants). When `VERIFIER_BASE_URL` is a local endpoint (`localhost`/`127.0.0.1`), an unset `PRIVATE_KEY`/`CERTIFICATE` defaults to the sample chain in `server/samples/certificate-openid-test/`, whose SAN is `localhost` — matching the `x509_san_dns:localhost` client id that `POST /request` falls back to when none is given. For any other base URL — including when repairing a deployed verifier by pointing `VERIFIER_BASE_URL` at its API URL — `PRIVATE_KEY`/`CERTIFICATE` are required and `initialize()` throws without them, since the bundled sample's private key is public and must never be registered against a deployed verifier.
