...
| `AUTHZ_BASE_URL` | Issuer **required**, Authz (optional) | Authorization Server issuer used by Issuer for access-token validation and `authorization_servers` metadata. Must match the token's `iss` exactly (no trailing slash). Also overrides the base URL used in Authz metadata (Authz default: `http://localhost:{AUTHZ_PORT}`) |
...
- **Authz public keys**: `/credentials` verifies access tokens signed by the Authorization Server, so the Issuer also registers `kmsAuthzSignatureKeyStore()`. The CDK stack grants `kms:DescribeKey` and `kms:GetPublicKey` on `alias/vcknots/authz/*` only (`grantSignatureKeyPublicKeyAccess()`). CreateKey and Sign on Authz keys stay on the Authz role.
...
- **Store drift**: the verifier metadata lives in DynamoDB while the key lives in KMS, so the two can drift apart — most often when an environment that previously ran on the in-memory key store is pointed at KMS. `createVerifierMetadata` rejects an already-registered verifier and cannot repair that, so the Verifier only logs a warning at startup (`Verifier metadata exists but no <alg> key is registered in KMS`) and keeps running. Recovery is manual, and it is the same procedure that seeds a verifier in the first place — note that **registration only runs locally**: `handlers/verifier.ts` skips `initialize()` when `AWS_LAMBDA_FUNCTION_NAME` is set, and there is no HTTP endpoint that registers a verifier, so a deployed Lambda never registers one by itself.
  ...
  1. Delete the verifier's item from the Verifiers table. Its partition key is not the client id itself but the hex MD5 of it: `node -e "console.log(require('crypto').createHash('md5').update('<client-id>').digest('hex'))"`.
...
- **Registration**: the certificate is written on first startup, from `PRIVATE_KEY_PATH`/`CERTIFICATE_PATH`. When `VERIFIER_BASE_URL` is a local endpoint (`localhost`/`127.0.0.1`), an unset `PRIVATE_KEY_PATH`/`CERTIFICATE_PATH` defaults to the sample chain in `server/samples/certificate-openid-test/`, whose SAN is `localhost` — matching the `x509_san_dns:localhost` client id that `POST /request` falls back to when none is given. For any other base URL — including when repairing a deployed verifier by pointing `VERIFIER_BASE_URL` at its API URL — `PRIVATE_KEY_PATH`/`CERTIFICATE_PATH` are required and `initialize()` throws without them, since the bundled sample's private key is public and must never be registered against a deployed verifier.
- **Deployed Lambdas do not self-register.** Initialization only runs outside Lambda (`server/aws/src/handlers/verifier.ts` skips it when `AWS_LAMBDA_FUNCTION_NAME` is set), and the secret name is derived from the verifier's base URL, so a certificate seeded from `http://localhost:8083` is not the one an API Gateway URL looks up. On a deployed stack, either write the secret manually or use a client id prefix that needs no certificate. `SecretString` must be a JSON array of PEM certificates (leaf first, then any intermediates), since the provider `JSON.parse`s it: `["-----BEGIN CERTIFICATE-----\n...\n-----END CERTIFICATE-----\n"]`.
...
- **Required IAM** (granted to the Authz Lambda role by the CDK stack): the same actions as the Issuer and Verifier, scoped to the `alias/vcknots/authz/*` namespace and to keys tagged `vcknots:authz-signature-key=true`. The Issuer role also gets `DescribeKey`/`GetPublicKey` on this namespace so `/credentials` can verify Authz-issued tokens; it cannot create or sign with Authz keys.
...
- **OAuth policy**: local `start:authz` also loads the `authorization_server` object from `server/samples/oauth-server.json` into the Authz OAuth policies table (`AUTHZ_OAUTH_POLICIES_TABLE_NAME`). It is inserted only when no policy is stored; an existing policy is not overwritten. This runs even when authz metadata already exists. A deployed Authz Lambda does not seed the policy — run `start:authz` once against the same table, or write the item yourself. The bundled sample sets `senderConstrainedAccessToken.dpop.mode` to `optional` for both `default_client` and `anonymous_client`. Issuer reads the same policy and client tables (it does not seed them) so the Credential and nonce endpoints can resolve DPoP the same way as Authz `/token`.
- **OAuth clients**: local `start:authz` also loads `server/samples/oauth-clients.json` into the Authz OAuth clients table (`AUTHZ_OAUTH_CLIENTS_TABLE_NAME`). Each `client_id` is inserted only if it is missing; existing rows are not overwritten. This runs even when authz metadata already exists. A deployed Authz Lambda does not seed clients — run `start:authz` once against the same table, or write items yourself.
  ...
  3. Run `pnpm start:authz` once. It registers the sample metadata and creates the KMS key for that authorization server, inserts the sample OAuth policy when none is stored, and inserts any missing sample OAuth clients, then you can stop it.
