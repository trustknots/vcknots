...
- **登録タイミング**: 初回起動時に `PRIVATE_KEY`/`CERTIFICATE`（または `*_PATH`）から書き込まれます。`VERIFIER_BASE_URL` がローカルエンドポイント（`localhost`/`127.0.0.1`）の場合に限り、未指定であれば `server/samples/certificate-openid-test/` の同梱サンプルにフォールバックします（このサンプル証明書の SAN は `localhost` で、`POST /request` が client_id 未指定時にフォールバックする `x509_san_dns:localhost` と一致します）。それ以外のベース URL の場合 — デプロイ済み verifier を修復するために `VERIFIER_BASE_URL` をその API URL に設定する場合を含む — `PRIVATE_KEY`/`CERTIFICATE` の指定が必須で、未指定だと `initialize()` は例外を送出します。同梱サンプルの秘密鍵は公開されているため、デプロイ済みの verifier に登録してはいけません。
