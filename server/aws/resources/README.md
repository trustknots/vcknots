...
The Issuer role also gets `DescribeKey` and `GetPublicKey` on `alias/vcknots/authz/*` via `grantSignatureKeyPublicKeyAccess()`, so `/credentials` can verify Authz-issued access tokens. CreateKey and Sign on Authz keys stay on the Authz role.

The Verifier role also has a scoped Secrets Manager policy for `secretsManagerVerifierCertificateStore` (see `lib/construct/api/verifier-api.ts`). `CreateSecret` has no resource-level permissions in Secrets Manager — the secret's ARN does not exist yet when the request is made — so it is granted on `Resource: '*'` with a `secretsmanager:Name` condition scoped to `vcknots/verifier-certificates/*`. `PutSecretValue`/`GetSecretValue` are granted in a separate statement restricted to `secret:vcknots/verifier-certificates/*`; the trailing wildcard is required because Secrets Manager appends a random six-character suffix to every secret ARN. The prefix constant is duplicated in the construct and in `aws/src/providers/secrets-manager.ts`, and is passed to the Lambda as `VERIFIER_CERTIFICATE_SECRET_PREFIX` so the two cannot drift apart. Secrets use the `aws/secretsmanager` managed key, so no KMS grant is needed.
