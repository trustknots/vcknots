import { CredentialConfigurationId, CredentialRequest, CredentialIssuer, initializeIssuerFlow, } from '@trustknots/vcknots/issuer';
import { AuthorizationServerIssuer, initializeAuthzFlow } from '@trustknots/vcknots/authz';
import { Hono } from 'hono';
import { handleError } from '../utils/error-handler.js';
export const createIssueRouter = (context, baseUrl) => {
    ...
    issueApp.post('/configurations/:configuration/offer', async (c) => {
        try {
            ...
            return c.text(`openid-credential-offer://?credential_offer=${encodeURIComponent(JSON.stringify(offer))}`);
        }
        catch (err) {
