...
| [13.2](https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0.html#section-13.2) | Client Authentication | `private_key_jwt` | Since `v0.6.0`<br />✅ Validate | ✅ Send | Client authentication method for registered OAuth clients at the Token Endpoint. The wallet defaults to anonymous (`none`); it sends a client assertion only when the wallet is configured for `private_key_jwt` and the Authorization Server Metadata advertises both `private_key_jwt` and the configured signing algorithm. A `token_endpoint_auth_methods_supported` that also lists `none` does not downgrade the configured method. An Authorization Server that omits `token_endpoint_auth_methods_supported` cannot be used at all, because RFC 8414 section 2 makes the omission mean `client_secret_basic`. Related section: [6.1](https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0.html#section-6.1) |
| [6.1](https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0.html#section-6.1) | Client Authentication | Anonymous Pre-Authorized Token Request | Since `v0.6.0`<br />✅ Conditional | ✅ Send | Available only when the wallet's `token_endpoint_auth_method` is `none`, the Authorization Server Metadata lists `none` in `token_endpoint_auth_methods_supported`, and `pre-authorized_grant_anonymous_access_supported` is `true`. That parameter defaults to `false`, so an Authorization Server that omits it receives a token request carrying `client_id`, and the request is refused before it is sent when no `client_id` is configured. Related section: [12.3](https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0.html#section-12.3) |
