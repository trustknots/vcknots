...
| [13.2](https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0.html#section-13.2) | Client Authentication | `private_key_jwt` | Since `v0.6.0`<br />✅ Validate | ✅ Send | Client authentication method for registered OAuth clients at the Token Endpoint. The wallet's default authentication method is `none` (no client authentication). The wallet sends a client assertion only when all of the following hold:<br />1. The wallet's `token_endpoint_auth_method` is `private_key_jwt`<br />2. The Authorization Server Metadata lists `private_key_jwt` in `token_endpoint_auth_methods_supported`<br />3. The same metadata lists the configured signing algorithm in `token_endpoint_auth_signing_alg_values_supported`<br />When condition 1 holds but condition 2 or 3 does not, the wallet does not switch to `none`, even if `token_endpoint_auth_methods_supported` lists `none`; it returns an error before sending the Token Request. The wallet cannot use an Authorization Server whose metadata omits `token_endpoint_auth_methods_supported`, because RFC 8414 section 2 treats the omission as `client_secret_basic`, which the wallet does not implement. Related section: [6.1](https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0.html#section-6.1) |
| [6.1](https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0.html#section-6.1) | Client Authentication | Anonymous Pre-Authorized Token Request | Since `v0.6.0`<br />✅ Conditional | ✅ Send | The wallet omits `client_id` from the Token Request (even when one is configured) only when all of the following hold:<br />1. The wallet's `token_endpoint_auth_method` is `none` (the default)<br />2. The Authorization Server Metadata lists `none` in `token_endpoint_auth_methods_supported`<br />3. The same metadata sets `pre-authorized_grant_anonymous_access_supported` to `true`<br />`pre-authorized_grant_anonymous_access_supported` defaults to `false`. When conditions 1 and 2 hold but that parameter is `false` or omitted, the wallet sends the Token Request with `client_id`, or returns an error before sending it if no `client_id` is configured. Related section: [12.3](https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0.html#section-12.3) |
