...
- `single: true` (シングルプロバイダー) : 同じ `kind` のプロバイダーが既に存在している場合、後から登録されたプロバイダーで **完全に上書き（置き換え）** されます。
...
- シングルプロバイダー (`single: true`) の場合: 登録されている唯一のプロバイダーインスタンスを 1 つ返します。
...
2. **遅延解決** : 他のプロバイダーが必要になった瞬間に Registry を経由して取得するため、無駄な初期化コストがなく、プロバイダーが動的に差し替えられた場合でも、常に最新の有効なインスタンスを安全に取得・参照できます。
