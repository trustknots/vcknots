...
describe('kmsAuthzSignatureKeyStore', () => {
  ...
  const createProvider = () =>
    kmsAuthzSignatureKeyStore({ client: kmsMock as unknown as KMSClient })
  ...
  it('should save an ES256 authz key by importing it into KMS', async () => {
    ...
    assert.deepEqual(kmsMock.commandCalls(CreateKeyCommand)[0]?.args[0].input, {
      ...
      Origin: 'EXTERNAL',
      Tags: [{ TagKey: AUTHZ_KEY_TAG_KEY, TagValue: 'true' }],
    })
  ...
  it('should fail save when the aliased key is not in a usable state', async () => {
    ...
    const provider = createProvider()
    await assert.rejects(provider.save(authz, 'ES256'), (error: Error) => {
      assert.equal(error.name, 'internal_server_error')
  ...
  it('should discard the generated key when CreateAlias fails outright', async () => {
    ...
    const provider = createProvider()
    await assert.rejects(provider.save(authz, 'ES256'), /access denied/)
  ...
  it('should reject an empty key algorithm before calling KMS', async () => {
    const provider = createProvider()
    await assert.rejects(provider.fetch(authz, '  '), (error: Error) => {
      assert.equal(error.name, 'internal_server_error')
      assert.match(error.message, /Authorization server key algorithm is required/)
      return true
    })
  ...
  it('should return null from fetch when the key does not exist', async () => {
    ...
    const provider = createProvider()
    assert.equal(await provider.fetch(authz, 'ES256'), null)
  })
  ...
  it('should rethrow non-NotFound errors from DescribeKey on fetch', async () => {
    ...
    const provider = createProvider()
    await assert.rejects(provider.fetch(authz, 'ES256'), /access denied/)
  })

  it('should sign an ES256 access token and convert the DER signature to JOSE format', async () => {
    ...
    const jwtHeader = { alg: 'ES256', typ: 'at+jwt' }
    const jwtPayload = { iss: authz, sub: 'client-1' }
    ...
    assert.equal(signInput?.KeyId, authzKeyAlias('ES256'))
  ...
  it('should sign an RS256 access token and return the raw signature as base64url', async () => {
    ...
    const provider = createProvider()
    const result = await provider.sign(authz, 'RS256', { iss: authz, sub: 'client-1' }, {
      alg: 'RS256',
      typ: 'at+jwt',
    })
